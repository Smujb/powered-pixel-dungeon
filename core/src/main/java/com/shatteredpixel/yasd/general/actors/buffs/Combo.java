...
public class Combo extends Buff implements ActionIndicator.Action {
	...
	public Image getIcon() {
		Image icon;
		if (((Hero)target).belongings.getWeapons().get(0) != null){//If the hero has any getWeapons equipped, use the first one's image.
			icon = new ItemSprite(((Hero)target).belongings.getWeapons().get(0).image, null);
		} else {
	...
	private CellSelector.Listener finisher = new CellSelector.Listener() {
		...
		private void doAttack(final Char enemy){
			...
			Char.DamageSrc src =  new Char.DamageSrc(Element.PHYSICAL, target);
			boolean wasAlly = enemy.alignment == target.alignment;
			dmg = enemy.defenseProc(target, dmg, src);
							...
							for (int i = 0; i < PathFinder.NEIGHBOURS8.length; i++) {
								...
								if (enemy.pos - target.pos == ofs) {
									...
									if ((Dungeon.level.passable(newPos) || Dungeon.level.avoid(newPos))
											...
											&& enemy.canOccupy(Dungeon.level, newPos)) {
