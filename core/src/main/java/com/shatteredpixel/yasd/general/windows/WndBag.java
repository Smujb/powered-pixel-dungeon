...
public class WndBag extends WndTabbed {
	...
	public boolean onSignal(KeyEvent event) {
		if (event.pressed && KeyBindings.getActionForKey( event ) == PPDAction.INVENTORY) {
			hide();
			return true;
		} else {
				...
				if (item.name() == null) {
					...
					enable(
							mode == Mode.FOR_SALE && !item.unique && (item.price() > 0) && (!item.isEquipped(Dungeon.hero) || !item.cursed) ||
									...
									mode == Mode.ALL ||
									mode == Mode.INCREASE_TIER && ((item instanceof MeleeWeapon && ((MeleeWeapon) item).tier < Dungeon.maxTier() && !(item instanceof MagesStaff)) || (item instanceof Armor && ((Armor) item).tier < Constants.MAXIMUM_TIER))
					);
				}
			} else {
				bg.color( NORMAL );
