...
public class ScrollOfRetribution extends Scroll {
	...
	public void doRead() {
		...
		for (Mob mob : Dungeon.level.mobs.toArray( new Mob[0] )) {
			if (Dungeon.level.heroFOV[mob.pos]) {
				...
				mob.damage(Math.round(mob.HT/10f + (mob.HP * power * 0.225f)), new Char.DamageSrc(Element.SPIRIT, this).ignoreDefense());
				if (mob.isAlive()) {
	...
	public void empoweredRead() {
		...
		for (Mob mob : Dungeon.level.mobs.toArray( new Mob[0] )) {
			if (Dungeon.level.heroFOV[mob.pos]) {
				mob.damage(Math.round(mob.HP * power/5f), new Char.DamageSrc(Element.SPIRIT, this).ignoreDefense());
			}
		}
