                    ...
                    if (cur[cell] > 0 && (ch = Actor.findChar( cell )) != null) {
                        if (!ch.isImmune(this.getClass())) {
                            if (ch instanceof Hero) {
                                ch.damage(Random.Int(Math.max(1,strength/2), strength+2), true, new Char.DamageSrc(Element.DARK, this) );//Take some direct damage, cap scaling with max HP and never 0. Also prevents the hero standing in it for bonus shielding/sneakSkill without consequence
                            } else {
