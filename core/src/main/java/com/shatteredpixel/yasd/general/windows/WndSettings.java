...
public class WndSettings extends WndTabbed {
	...
	public WndSettings() {
		...
		//TODO finish CPD log system and port to PPD
		...
		public DebugTab() {
			...
			RedButton btnCopyLog = new RedButton(Messages.get(this, "copy_log")) {
				...
				protected void onClick() {
					...
					//clipboard.setContents(PPDGame.getLog());
				}
			};
	...
	private static class DisplayTab extends Component {
		...
		protected void createChildren() {
			...
			optBrightness.setSelectedValue(PPDSettings.brightness());
			add(optBrightness);
			...
			optVisGrid.setSelectedValue(PPDSettings.visualGrid());
			add(optVisGrid);
		...
		protected void layout() {
			...
			height = chkInterlevelScene.bottom();
		}
	...
	private static class UITab extends Component {
		...
		protected void createChildren() {
			...
			if (PPDSettings.toolbarMode().equals(Toolbar.Mode.SPLIT.name()))
			...
			if (PPDSettings.toolbarMode().equals(Toolbar.Mode.GROUP.name()))
			...
			if (PPDSettings.toolbarMode().equals(Toolbar.Mode.CENTER.name()))
	...
	private static class DataTab extends Component{
		...
		protected void createChildren() {
			...
			chkNews = new CheckBox(Messages.get(this, "news")){
				...
				protected void onClick() {
					super.onClick();
					PPDSettings.news(checked());
					//News.clearArticles();
				}
			};
			chkNews.checked(PPDSettings.news());
			add(chkNews);
			...
			chkUpdates = new CheckBox(Messages.get(this, "updates")){
				...
				protected void onClick() {
					super.onClick();
					PPDSettings.updates(checked());
					Updates.clearUpdate();
				}
			};
			chkUpdates.checked(PPDSettings.updates());
			add(chkUpdates);
			...
			if (!DeviceCompat.isDesktop()){
				chkWifi = new CheckBox(Messages.get(this, "wifi")){
					...
					protected void onClick() {
						super.onClick();
						PPDSettings.WiFi(checked());
					}
				};
				chkWifi.checked(PPDSettings.WiFi());
				add(chkWifi);
			}
		}
	...
	private static class AudioTab extends Component {
		...
		protected void createChildren() {
			...
			optMusic.setSelectedValue(PPDSettings.musicVol());
			add(optMusic);
			...
			chkMusicMute.checked(!PPDSettings.music());
			add(chkMusicMute);
			...
			optSFX.setSelectedValue(PPDSettings.SFXVol());
			add(optSFX);
			...
			chkMuteSFX.checked(!PPDSettings.soundFx());
			add( chkMuteSFX );
				...
				RedButton btn = new RedButton(Messages.titleCase(langs.get(i).nativeName()), 8){
					...
					protected void onClick() {
						...
						PPDGame.seamlessResetScene(new Game.SceneChangeCallback() {
							@Override
							public void beforeCreate() {
								PPDSettings.language(langs.get(langIndex));
								GameLog.wipe();
								Game.platform.resetGenerators();
							}
							@Override
			...
			if (currLang != Languages.ENGLISH) {
				...
				btnCredits = new RedButton(credText, credText.length() > 9 ? 6 : 9) {
					...
					protected void onClick() {
						...
						PPDGame.scene().addToFront(credits);
					}
				};
				add(btnCredits);
			}
