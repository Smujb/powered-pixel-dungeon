...
import com.shatteredpixel.yasd.items.food.MeatPie;
import com.shatteredpixel.yasd.items.food.MysteryMeat;
import com.shatteredpixel.yasd.items.food.Pasty;
import com.shatteredpixel.yasd.items.food.StewedMeat;
import com.shatteredpixel.yasd.items.potions.AlchemicalCatalyst;
import com.shatteredpixel.yasd.items.potions.Potion;
import com.shatteredpixel.yasd.items.potions.brews.BlizzardBrew;
import com.shatteredpixel.yasd.items.potions.brews.CausticBrew;
import com.shatteredpixel.yasd.items.potions.brews.InfernalBrew;
import com.shatteredpixel.yasd.items.potions.brews.ShockingBrew;
import com.shatteredpixel.yasd.items.potions.elixirs.ElixirOfAquaticRejuvenation;
import com.shatteredpixel.yasd.items.potions.elixirs.ElixirOfArcaneArmor;
import com.shatteredpixel.yasd.items.potions.elixirs.ElixirOfDragonsBlood;
import com.shatteredpixel.yasd.items.potions.elixirs.ElixirOfHoneyedHealing;
import com.shatteredpixel.yasd.items.potions.elixirs.ElixirOfIcyTouch;
import com.shatteredpixel.yasd.items.potions.elixirs.ElixirOfMight;
import com.shatteredpixel.yasd.items.potions.elixirs.ElixirOfToxicEssence;
import com.shatteredpixel.yasd.items.potions.exotic.ExoticPotion;
import com.shatteredpixel.yasd.items.scrolls.Scroll;
import com.shatteredpixel.yasd.items.scrolls.exotic.ExoticScroll;
import com.shatteredpixel.yasd.items.spells.Alchemize;
import com.shatteredpixel.yasd.items.spells.AquaBlast;
import com.shatteredpixel.yasd.items.spells.ArcaneCatalyst;
import com.shatteredpixel.yasd.items.spells.BeaconOfReturning;
import com.shatteredpixel.yasd.items.spells.CurseInfusion;
import com.shatteredpixel.yasd.items.spells.FeatherFall;
import com.shatteredpixel.yasd.items.spells.MagicalInfusion;
import com.shatteredpixel.yasd.items.spells.MagicalPorter;
import com.shatteredpixel.yasd.items.spells.PhaseShift;
import com.shatteredpixel.yasd.items.spells.ReclaimTrap;
import com.shatteredpixel.yasd.items.spells.Recycle;
import com.shatteredpixel.yasd.items.spells.WildEnergy;
...
import com.shatteredpixel.yasd.windows.WndInfoItem;
...
public class QuickRecipe extends Component {
	...
	public QuickRecipe(Recipe r, ArrayList<Item> inputs, final Item output) {
		...
		for (final Item in : inputs) {
			...
			curr = new ItemSlot(in) {
				...
				protected void onClick() {
					YASD.scene().addToFront(new WndInfoItem(in));
				}
			};
		...
		if (hasInputs) {
			...
			if (!(YASD.scene() instanceof AlchemyScene)) {
				arrow.enable(false);
			}
		} else {
		...
		this.output = new ItemSlot(output){
			@Override
			protected void onClick() {
				YASD.scene().addToFront(new WndInfoItem(output));
			}
		};
		if (!hasInputs){
	...
	public class arrow extends IconButton {
		...
		protected void onClick() {
			...
			((AlchemyScene) YASD.scene()).populate(ingredients, Dungeon.hero.belongings);
		}
